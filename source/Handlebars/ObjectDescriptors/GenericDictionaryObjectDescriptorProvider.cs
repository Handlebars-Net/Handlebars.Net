...
{
    public sealed class GenericDictionaryObjectDescriptorProvider : IObjectDescriptorProvider
    {
        private static readonly MethodInfo CreateDescriptorMethodInfo =
            new Func<ObjectDescriptor>(CreateDescriptor<IDictionary<object, object>, object, object>)
            ...
            where TK : notnull
        {
            return new ObjectDescriptor(
                typeof(T),
                new GenericDictionaryAccessor<T, TK, TV>(),
                (descriptor, o) => ((T) o).Keys,
                self => new DictionaryIterator<T, TK, TV>()
            );
        }
    }
}